...
function createEventListeners() {
    ...
    // manage CSS classes to get to menu option 
    ...
    // return to game being played without resetting it 
...
/**This function evaluates the board to see if a draw has been reached */
...
function checkForWin(colourW) {
    ...
    for (let i = 0; i < 7; i++) {
        for (let k = 0; k < 3; k++) {
            if (slots[i + (7 * k)].classList.contains(currentColour) && slots[i + (7 * k) + 7].classList.contains(currentColour) && slots[i + (7 * k) + 14].classList.contains(currentColour) && slots[i + (7 * k) + 21].classList.contains(currentColour)) return showResults(false);
        }
    }
    ...
    for (let i = 0; i < leftRightStartingPoints.length; i++) {
        ...
        if (slots[k].classList.contains(currentColour) && slots[k + 8].classList.contains(currentColour) && slots[k + 16].classList.contains(currentColour) && slots[k + 24].classList.contains(currentColour)) return showResults(false);
    }
    ...
    for (let i = 0; i < rightLeftStartingPoints.length; i++) {
        ...
        if (slots[k].classList.contains(currentColour) && slots[k + 6].classList.contains(currentColour) && slots[k + 12].classList.contains(currentColour) && slots[k + 18].classList.contains(currentColour)) return showResults(false);
    }
}
...
function computerNextMove() {
    ...
    let nextMove = 3; //default to center column move
